...
public class Q_coffee {
	...
	public static void main(String[] args) {
		...
		int expences = 0;// 지출
		//Coffee():생성자
		//         Coffee타입의 인스턴스를 생성할 떄 사용하는 특수한 메서드 
		Coffee americano = new Coffee(); //인스턴스 
		...
		Coffee mocha = new Coffee("모카", 4000, 3000, 10,30,0);
		
		Coffee latte = new Coffee("라떼",5000,4000,10,5,0);
		
		Coffee[] drinks = {americano, mocha, latte};
...
//			         - 커피 재고가 안전재고 미만이 되면 안전재고의 두 배 만큼 매입한다.
				if (drinks[inputCode].stock < drinks[inputCode].safetystock) {
					...
					System.out.println(" 결재금액 :" + (drinks[inputCode].price * orderCnt));
					System.out.println(" 남은재고 : " + drinks[inputCode].stock);
				}
