...
public class Run {
		...
		public static void main(String[] args) {
			...
			// 실행 대기  	: Thread가 실행될 수 있는데, 아직 스케쥴링이 되지 않아서 실행을 기다리는 상태.
			// 일시정지 	: Thread를 실행할 수 없는 상태.
			//				1) waiting		 : Thread.join(),Object.wati() 메서드에 의해 멈춰진 상태.
			...
			// join()  		: 쓰래드가 종료될 때까지 해당 쓰래드를 호출한 쓰래드를 일시정지상태로 변경.
			// interrupt()  : join() 또는 sleep() 에 의해 일시정지 상태가 된 쓰래드를 실행대기로 변경
			// Object.wait(): 쓰래드를 일시정지 
			// Object.notify() : Object.wait()에 의해서 
			 ...
			 m2.setName("m2");
			 //쓰래드를 시작, 쓰래드가 생성되고 실행될 준비가 끝나면 JVM이 run()을 실행
			 //m2.start();
			 ...
			 for (int i = 0; i < 100; i++) {
			 	...
			 	if(i == 50) {
			 		 try {
						 //main 쓰래드를 일시정지 상태로 만듬
						 //m2의 쓰래드가 실행이 끝나면 main 쓰래드 
						m2.join();
