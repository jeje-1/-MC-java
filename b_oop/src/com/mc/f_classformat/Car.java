...
public class Car {
	...
	// 					값을 명시적으로 할당하지 않으면, JVM의 기본값으로 초기화
	
	//                  인스턴스 변수: non static 변수, 
	//								클래스가 인스턴스화 되는 시점에 초기화 되는 변수
	// 								같은 클래스이더라도 인스턴스마다 값이 개별적이다.
	...
	// 초기화 순서 
	// 인스턴스 변수 : jvm기본값 > 직접 할당한 값 > 초기화 블록 > 생성자
	// 클래스 변수 : jvm기본값 > 직접 할당한 값 > static 초기화 블록
	...
	//인스턴스 변수
		...
		private static int afterServicePeriod = 0;
	...
	static {
		...
		System.out.println("staitc 초기화 블록 이후 값 : " + afterServicePeriod);
		System.out.println("================================================");
	...
	public Car(String name, String color){
		...
		this.name = name;
		this.color = color;
		System.out.println("this에 담긴 값을 확인 : " + this);
		System.out.println("super에 담긴 값을 확인 : " + super.toString());
	}
	
	//캡슐화
	// 클래스에서 숨길 것과 드러낼 것을 결정하여 추상성을 확보하는 과정
	// 필드변수 : 필드변수를 접근제한자를 private으로 지정하고 외부에서 필드변수에 직접 접근하는 것을 차단하고
	//			getter/setter를 통해서만 필드변수에 접근하는 것을 허용하는 것.
	//			멤버변수에 대한 접근권한을 관리할 수 있다.
	// 			getter/setter : 읽기,쓰기  getter : 읽기  getter/setter 미작성 : 접근 금지
	// 메서드 : 외부에서 메세지를 전달받는 용도의 public메서드와, 세부적인 기능을 구현하는 private메서드를 분리
	//		  클래스간 결합도를 낮춰 유연한 설계를 가능하세 해줌
	...
	//Override : 부모클래스부터 상속받은 메서드를 재정의 하는 것
	//			- 부모클래스에 선언된 메서드의 접근제한자보다 넓은범위의 접근제한자로 변경할 수 있다.
	...
	//			- static메서드는 상속이 안된다. 따라서 Override도 안된다. 
