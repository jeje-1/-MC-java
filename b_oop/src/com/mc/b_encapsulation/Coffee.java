...
public class Coffee {
	...
	// getter만 만들면 ? : 읽기만 가능
	// getter/setter를 만들면 ? : 읽고 쓰기 둘다 가능
	// getter/setter 안 만들면 ? : 읽고 쓰기 모두 불가능

	// 만약 캡슐화가 없다면?
	// 외부에서 cost에 -100000000 을 넣었을 떄 방어할 방법이 있을까?
	// setter메서드를 사용하면, 값에 대한 검증이 가능
	...
	private Coffee(String name, int price, int cost, int stock, int safetystock, int salesCnt) {
		// this : heap 영역에 생성된 인스턴스의 메모리 주소를 저장하고 있는 참조형 변수
		this.name = name;
		this.price = price;
		this.cost = cost;
		this.stock = stock;
